...
class MyCylinderNoCovers extends CGFobject
{
	/**
	 * 
	 * @param {*} scene 
	...
	constructor(scene, base, top, height, slices, stacks){
		...
		// initialize the top and bottom covers (MyCircle)
	...
	initBuffers(){
		...
		for(let i = 0; i <= this.slices; i++) {
			for(let j = 0; j <= this.stacks; j++) {
				...
				this.normals.push(currRadius*Math.cos(theta*i),currRadius*Math.sin(theta*i), j*heightPerStack);
